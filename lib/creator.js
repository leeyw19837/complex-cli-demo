...
class Creator {
  ...
  download = async (branch) => {
    ...
    // 把资源下载到指定的本地磁盘的文件夹
    const localCacheFolder = this.dir;
    // 指定文件夹的模板的路径
    this.templateDir = path.join(localCacheFolder, 'react', 'react+js')
    // 判断是否已经下载过该模板
    const hasDownloaded = fs.exists(this.templateDir);
    if (hasDownloaded) {
  ...
  create = async () => {
    ...
    // 3 将模板复制到目标目录
    console.log('$$$', this.templateDir, this.dir)
